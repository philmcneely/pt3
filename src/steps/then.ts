...
import getRequests from '../support/action/getRequests';
import checkEvent from '../support/check/checkEvent';
import checkEvent48 from '../support/check/checkEvent48';

Then(
    /^I expect that at least a single event is recorded$/,
    getRequests
);

Then(
    /^I expect that an event is recorded$/,
    getRequests
);

Then(
    /^I expect that event "([^"]*)?" is recorded$/,
    checkEvent
);

Then(
    /^I expect that event "([^"]*)?" is recorded with args "([^"]*)?"$/,
    checkEvent
);
